...
function emailReserver(response, detail){
	var mailOptions = {
	    ...
	    html:"<a href='http://localhost:8080/cancel/"+detail[0]+"'> Cancel Request </a>"
	};
	...
	emailer.send(mailOptions, function(error){
		if (error === null){
			...
			// the email might be unreachable
			response.send('Your email address is unreachable!');
		}
	});
}
...
function cancel(data, response){
	...
	var mailOptions = {
	    ...
	    html: 
	    "<p>Hello " + data.ownerName+' and '+data.name+' </p> <p></p>'+
	    "<p>Our meeting from "+data.startTime+" to "+data.endTime+" has been cancelled.</p> <p></p>"+
	    "<p>Regards,</p>"+
	    "Meet.My.Professor"
	};
